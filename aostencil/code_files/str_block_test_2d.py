...
static void *thread_func_@cpu_id(void *arg)
{
  ...
  if (@numa_in_id == 0)
  {
    sublattice[0] = (datatype *)numa_alloc_onnode(sizeof(datatype) * ynum * col, @numa_id);
    sublattice[1] = (datatype *)numa_alloc_onnode(sizeof(datatype) * ynum * col, @numa_id);

    if (sublattice[0] == NULL || sublattice[1] == NULL)
    {
      puts("Running out of memory!");
      printf("sublattice[%d] row:%d, col:%d", @numa_id, row, col);
      exit(-1);
    }
  }
  ...
  if (@numa_in_id == 0)
  {
    numa_free(sublattice[0], sizeof(datatype) * ynum * col);
    numa_free(sublattice[1], sizeof(datatype) * ynum * col);
  }
  return NULL;
}
'''
